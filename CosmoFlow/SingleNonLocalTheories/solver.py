...
class solver():
	...
	def __init__(self, Nspan, interpolated, Rtol, Atol):
		...
		self.initial = model(self.N_init, self.interpolated)
	...
	def Sigma_Re_init(self, k):
		...
		#Speed of sound
		mdl = model(N = self.Nspan, interpolated = self.interpolated)
	...
	def fffCalc(self, k1, k2, k3):
		...
		Bf123 = self.Xffp*self.initial.B_abc_fast(k1, k2, k3)
		Bf132 = self.Xffp*self.initial.B_abc_fast(k1, k3, k2)
		Bf231 = self.Xffp*self.initial.B_abc_fast(k2, k3, k1)

		Bs123 = self.Xffp*self.initial.B_abc_slow(k1, k2, k3)
		Bs132 = self.Xffp*self.initial.B_abc_slow(k1, k3, k2)
		Bs231 = self.Xffp*self.initial.B_abc_slow(k2, k3, k1)
			...
			for j in range(Nfield):
				for k in range(Nfield):
					fff[i, j, k] += 1/(a*a*a*a)/4/(k1*k2*k3)/kt*(-C123[i, j, k]*k1*k2 - C132[i, k, j]*k1*k3 - C231[j, k, i]*k2*k3
													...
													- a*a*Hi*Bs123[i, j, k]/2/k1/k2 * (K2 - k3*(k1+k2)) * 2
													- a*a*Hi*Bs132[i, k, j]/2/k1/k3 * (K2 - k2*(k1+k3)) * 2
													- a*a*Hi*Bs231[j, k, i]/2/k2/k3 * (K2 - k1*(k2+k3)) * 2
													+ D123[i, j, k]*Hi*(K2 - 2*k1*k2*k2/kt)
													+ D132[i, k, j]*Hi*(K2 - 2*k1*k2*k2/kt)
													+ D231[j, k, i]*Hi*(K2 - 2*k1*k2*k2/kt)
													- Bf123[i, j, k]/2*Hi/k1/k2 * (K2 - 1/kt*(2*k1*k2*k3 + kt*(k1+k2)*k3)) * 2
													- Bf132[i, k, j]/2*Hi/k1/k3 * (K2 - 1/kt*(2*k1*k2*k3 + kt*(k1+k3)*k2)) * 2
													- Bf231[j, k, i]/2*Hi/k2/k3 * (K2 - 1/kt*(2*k1*k2*k3 + kt*(k2+k3)*k1)) * 2
													)
		...
		for i in range(Nfield):
			for j in range(Nfield):
				for k in range(Nfield):
					...
					pff[i, j, k] += - 1./(a*a*a)/4./K3 * Hi * (-k1*k1*k2*k3/kt) * (Bs123[i, j, k]/2/Hi*k1*k2*k3*k3 * 2
																	+ Bs132[i, k, j]/2/Hi*k1*k2*k3*k2 * 2
																	+ Bs231[j, k, i]/2/Hi*k1*k2*k3*k1 * 2
																	+ Bf123[i, j, k]/2/Hi*k1*k2*k3*k3/a/a * 2
																	+ Bf132[i, k, j]/2/Hi*k1*k2*k3*k2/a/a * 2
																	+ Bf231[j, k, i]/2/Hi*k1*k2*k3*k1/a/a * 2
																	)

					pff[i, j, k] += - Hi/4/a/a/a/(k1*k2*k3)**2/kt * (-k1**2*(k2+k3)) * ( Bs123[i, j, k]/2*a*a*Hi * (k1+k2)*k3**2/(k1*k2*k3) * 2
																					   + Bs132[i, k, j]/2*a*a*Hi * (k1+k3)*k2**2/(k1*k2*k3) * 2
																					   + Bs231[j, k, i]/2*a*a*Hi * (k2+k3)*k1**2/(k1*k2*k3) * 2
																					   #+ Bf123[i, j, k]/2*Hi * k3/(k1*k2*k3)/kt * (2*k1*k2*k3 + kt*(k1+k2)*k3) * 2
																					   #+ Bf132[i, k, j]/2*Hi * k2/(k1*k2*k3)/kt * (2*k1*k2*k3 + kt*(k1+k3)*k2) * 2
																					   #+ Bf231[j, k, i]/2*Hi * k1/(k1*k2*k3)/kt * (2*k1*k2*k3 + kt*(k2+k3)*k1) * 2
			...
			for j in range(Nfield):
				for k in range(Nfield):
					ppf[i, j, k] += -1./(a*a*a*a)/4./K3 * (k1*k2*k3)*(k1*k2*k3)/kt*k1*k2*(-C123[i, j, k]*k1*k2 - C132[i, k, j]*k1*k3 - C231[j, k, i]*k2*k3
																	+ a*a*As123[i, j, k] + a*a*As132[i, k, j] + a*a*As231[j, k, i]
																	+ a*a*Hi*Bs123[i, j, k]*(k1+k2)/2 * k3*k3/k1/k2/k3 * 2
																	+ a*a*Hi*Bs132[i, k, j]*(k1+k3)/2 * k2**2/k1/k2/k3 * 2
																	+ a*a*Hi*Bs231[j, k, i]*(k2+k3)/2 * k1**2/k1/k2/k3 * 2
																	#+ Hi*Bf123[i, j, k]/2 * k3/(k1*k2*k3)/kt * (2*k1*k2*k3 + kt*(k1+k2)*k3) * 2
																	#+ Hi*Bf132[i, k, j]/2 * k2/(k1*k2*k3)/kt * (2*k1*k2*k3 + kt*(k1+k3)*k2) * 2
																	#+ Hi*Bf231[j, k, i]/2 * k1/(k1*k2*k3)/kt * (2*k1*k2*k3 + kt*(k2+k3)*k1) * 2
					...
					ppf[i, j, k] += -1/(a*a*a*a)/(k1*k2*k3)/kt * (k1**2*k2**2) * (- a*a*Hi*Bs123[i, j, k]*k3/2 * 2
																				  - a*a*Hi*Bs132[i, k, j]*k2/2 * 2
																				  - a*a*Hi*Bs231[j, k, i]*k1/2 * 2
																				  #- Hi*Bf123[i, j, k]*k3/2 * 2
																				  #- Hi*Bf132[i, k, j]*k2/2 * 2
																				  #- Hi*Bf231[j, k, i]*k1/2 * 2
																					)
		...
		for i in range(Nfield):
			for j in range(Nfield):
				for k in range(Nfield):
					...
					ppp[i, j, k] += - 1./(a*a*a)/4./K3 * Hi * (k1*k1*k2*k2*k3*k3)/kt * (Bs123[i, j, k]/2/Hi*k1*k2*k3*k3 * 2
																+ Bs132[i, k, j]/2/Hi*k1*k2*k3*k2 * 2
																+ Bs231[j, k, i]/2/Hi*k1*k2*k3*k1 * 2
																+ Bf123[i, j, k]/2/Hi/a/a*k1*k2*k3*k3 * 2
																+ Bf132[i, k, j]/2/Hi/a/a*k1*k2*k3*k2 * 2
																+ Bf231[j, k, i]/2/Hi/a/a*k1*k2*k3*k1 * 2
																)
	...
	def df_dN(self, y, N, k1, k2, k3):
		...
		mdl = model(N = N, interpolated = self.interpolated)
		uAB_k1 = mdl.u_AB(k1)
		uAB_k2 = mdl.u_AB(k2)
		uAB_k3 = mdl.u_AB(k3)
